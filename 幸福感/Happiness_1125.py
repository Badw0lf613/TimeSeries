...
# 绘制散点图查看样本分布状况
plt.scatter(train['id'],train['birth'])
...
data['nationality'] = data['nationality'].replace(-8, 1) # 将民族字段以-8填充的修改为汉族1

data.loc[data['religion'] < 0, 'religion'] = 1 #religion字段中有三种类型的负值，用这种方式将其修改为1(不信教)
data.loc[data['religion_freq'] < 0, 'religion_freq'] = 1 # 以1填充代表从未参加宗教活动

data.loc[data['edu'] < 0, 'edu'] = 0 #edu字段-8的修正为0（可以修正为平均值）

data.loc[data['income'] < 0, 'income'] = 0 # 将income=-8的修正为0（平均值会不会更好）

data.loc[data['political'] < 0, 'political'] = 1 # 修改政治面貌 -8--> 1 群众

# house=0 的有705人 但其中704人有住宅面积 其中1人无住宅面积
# 这里填充这个住宅面积=0的人 用704人的平均住宅面积代替==74.8
data['floor_area'] = data['floor_area'].replace(0, 74.8)
'''
# 检查女性身高均值  将女性身高为1的错误数据修正为均值（身高中所有为1的全是女性）
# 对应的 有47名女性的体重也为1，修正为女性体重的均值
# 对于身高高于150但体重少于70斤的字段 将其从公斤转为斤 注意到联合判断时需要对每个条件加上（）
'''
women_height_mean = data[(data['gender'] == 1)]['height_cm'].mean()
data['height_cm'] = data['height_cm'].replace(1, women_height_mean)

women_weight_mean = data[data['gender'] == 1]['weight_jin'].mean()
data['weight_jin'] = data['weight_jin'].replace(1, women_weight_mean)
data.loc[(data['weight_jin'] <= 70) & (data['height_cm'] >= 150), 'weight_jin'] = data['weight_jin'] * 2

data['health'] = data['health'].replace(-8, 3)# 对-8填充的缺失health字段以3填充
data['health_problem'] = data['health_problem'].replace(-8, 4)# 对health_problem=-8的以众数填充

data['depression'] = data['depression'].replace(-8, 3)# 感到沮丧程度 -8 用3修正

# 修正以-8填充的字段
data['socialize'] = data['socialize'].replace(-8, 3)
data['relax'] = data['relax'].replace(-8, 3)
data['learn'] = data['learn'].replace(-8, 3)
data['equity'] = data['equity'].replace(-8, 3)# 社会态度

data['class'] = data['class'].replace(-8, 5)# 阶级认同

# 用1填充人数为负数的字段，将家庭人数为50的修改为平均值
data.loc[data['family_m'] < 0, 'family_m'] = 1
data['family_m'] = data['family_m'].replace(50, 3)

# 划分是否有车 0、1标记，并生成one-hot变量
data['car'] = data['car'].replace(-8, 2)
data['status_peer'] = data['status_peer'].replace(-8, 2)
data['status_3_before'] = data['status_3_before'].replace(-8, 2)

data['view'] = data['view'].replace(-8, 3)# 社会认知

data['inc_ability'] = data['inc_ability'].replace(-8, 2)# 收入是否合理
